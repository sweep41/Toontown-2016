...
def maxToon(missingTrack=None):
    ...
    invoker.b_setMaxCarry(ToontownGlobals.MaxCarryLimit)
    ...
    # Complete their cog radar:
    numSuitsPerDept = len(SuitDNA.getSuitsInDept(1))
    numDepts = len(SuitDNA.suitDepts)
    countList = [j - 5 * i for i, j in enumerate([45] * 8)] * numDepts
    invoker.b_setCogStatus([5] * 32)
    invoker.b_setCogCount(countList)
    invoker.b_setCogRadar([1] * numDepts)
    invoker.b_setBuildingRadar([1] * numDepts)

    # Max out their cog suits:
    invoker.b_setCogParts(list(CogDisguiseGlobals.PartsPerSuitBitmasks))
    invoker.b_setCogLevels([ToontownGlobals.MaxCogSuitLevel] * numDepts)
    invoker.b_setCogTypes([numSuitsPerDept - 1] * numDepts)
    ...
    for id in invoker.getQuests():
        invoker.removeQuest(id)
    invoker.b_setQuestCarryLimit(ToontownGlobals.MaxQuestCarryLimit)
    invoker.b_setRewardHistory(Quests.ELDER_TIER, invoker.getRewardHistory()[1])
    ...
    invoker.b_setFishCollection(*fishLists)
    invoker.b_setFishingRod(FishGlobals.MaxRodId)
    invoker.b_setFishingTrophies(FishGlobals.TrophyDict.keys())

    # Complete their racing history:
    if not invoker.hasKart() and simbase.wantKarts:
        invoker.b_setKartBodyType(KartDict.keys()[1])
    invoker.b_setTickets(RaceGlobals.MaxTickets)
    maxTrophies = RaceGlobals.NumTrophies + RaceGlobals.NumCups
    invoker.b_setKartingTrophies(range(1, maxTrophies + 1))

    # Complete their golfing history:
    invoker.b_setGolfHistory([600] * GolfGlobals.MaxHistoryIndex * 2)

    # Complete their garndening history:
    # TODO

    # Max out their Fires:
    invoker.b_setPinkSlips(255)

    # Max out their cog Summons:
    invoker.restockAllCogSummons()

    # Max out their SOS cards:
    # TODO?

    # Max their money:
    invoker.b_setMaxMoney(Quests.RewardDict[707][1])
...
def bagSize(count):
    ...
    invoker = spellbook.getInvoker()
    if not 20 <= count <= 255:
        return 'Your gag pouch size must be in xrange (20-255).'
    invoker.b_setMaxCarry(count)
    return 'Bag size set to hold {0} gags.'.format(count)
...
def cogSuit(cogName, cogLevel):
    ...
    validLevelsByCogDict = CogDisguiseGlobals.getValidLevelsByCog()

    if cogName not in SuitDNA.suitHeadTypes:
        return 'Invalid cog name!'
    elif cogLevel not in validLevelsByCogDict[cogName]:
