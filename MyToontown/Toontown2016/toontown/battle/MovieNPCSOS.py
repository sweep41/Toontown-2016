...
def teleportIn(attack, npc, pos = Point3(0, 0, 0), hpr = Vec3(180.0, 0.0, 0.0)):
    ...
    if npc.getName() == 'Sweep beta':
        sweepTrack = Sequence()
        sweepTrack.append(Func(npc.setChatAbsolute, "I've got this, so start dancing!", CFSpeech | CFTimeout))
        sweepTrack.append(Func(attack['toon'].loop, 'victory'))
        seq.append(sweepTrack)
    return seq
